...
class Scraper:
      ...
      def scrape(self) -> RawSale:
            ...
            return RawSale(
                  ...
                  customer_account_balance = self.get_customer_account_balance(),
      ...
      def _get_optional(self, selector: str) -> str | None:
      ...
      def get_salesperson(self) -> str:
            element = self.soup.select_one("ul.invoice-detail li:nth-of-type(4)")
      ...
      def get_invoice_datetime(self) -> str:
      ...
      def get_comment(self) -> str | None:
            return self._get_optional(".invoice-policy")

      def get_change_due(self) -> str | None:
            # Specific selector — not index-based
            element = self.soup.select(".invoice-footer-value.invoice-total:nth-of-type(4)")
            return element.get_text(strip=True) if element else None

      def get_items_sold(self) -> str | None:
            element = self.soup.select(".invoice-footer-value.invoice-total:nth-of-type(2)")
            return element.get_text(strip=True) if element else None

      def get_customer_account_balance(self) -> str | None:
            element = self.soup.select(".invoice-footer-value.invoice-total:nth-of-type(3)")
            return element.get_text(strip=True) if element else None